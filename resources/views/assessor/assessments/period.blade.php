<x-layouts.assessor>
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                ...
                <p class="text-base-content/70 mt-1">
                    ...
                    <x-ui.badge type="error" size="sm" class="ml-2">Ditutup</x-ui.badge>
                    @elseif($period->status === 'draft')
                    <x-ui.badge type="warning" size="sm" class="ml-2">Draft</x-ui.badge>
                    @else
    ...
    <x-ui.alert type="info" class="mb-4">
        Periode ini masih dalam tahap persiapan.
    </x-ui.alert>
    @endif
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
        ...
        $pending = $teachers->filter(fn($t) => !isset($existingAssessments[$t->id]) ||
        $existingAssessments[$t->id]->status === 'pending')->count();
        $draft = $teachers->filter(fn($t) => isset($existingAssessments[$t->id]) && $existingAssessments[$t->id]->status
        === 'draft')->count();
        $completed = $teachers->filter(fn($t) => isset($existingAssessments[$t->id]) &&
        in_array($existingAssessments[$t->id]->status, ['submitted', 'finalized']))->count();
        @endphp
