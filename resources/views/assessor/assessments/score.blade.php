            ...
            <div class="flex-1">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-2 mt-2 text-sm">
                    ...
                    <div>
                        ...
                        <x-ui.badge type="{{ $assessment->status === 'draft' ? 'warning' : 'ghost' }}" size="sm" class="ml-1">
                                        ...
                                        @switch($item->field_type)
                                                    ...
                                                    <div class="grid grid-cols-2 md:grid-cols-{{ min($item->scale->options->count(), 5) }} gap-2">
                                                        @foreach($item->scale->options as $option)
                                                            <label class="cursor-pointer">
                                                                ...
                                                                <div class="p-3 rounded-lg border-2 border-base-300 peer-checked:border-primary peer-checked:bg-primary/10 text-center transition-all hover:border-primary/50">
                                                ...
                                                @elseif($item->options->isNotEmpty())
                                                    ...
                                                    <div class="space-y-2">
                                                        @foreach($item->options as $option)
                                                            <label class="flex items-center gap-3 p-3 rounded-lg border border-base-300 cursor-pointer hover:bg-base-200 transition-colors">
                                                                <input type="radio"
                                                                       name="values[{{ $item->id }}]"
                                                                       value="{{ $option->value }}"
                                            ...
                                            @default
                                                ...
                                                <input type="number"
                                                       ...
                                                       min="{{ $item->min_value ?? 0 }}"
                                                       max="{{ $item->max_value ?? 100 }}"
                                                       value="{{ $existingValue?->value_number ?? '' }}"
                ...
                <x-ui.card class="!shadow-2xl border border-base-300">
                    <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
                        ...
                        <div class="flex gap-2">
                            ...
                            <button type="button" class="btn btn-outline btn-primary" @click="saveDraft()" :disabled="isSaving">
                                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        function assessmentForm() {
            return {
                ...
                updateProgress() {
                    ...
                    const inputs = form.querySelectorAll('input[name^="values["], textarea[name^="values["], select[name^="values["]');
                    let filled = 0;
                    ...
                    form.querySelectorAll('input[type="number"][name^="values["], input[type="range"][name^="values["], textarea[name^="values["]').forEach(input => {
                ...
                async saveDraft(silent = false) {
                    ...
                    try {
                        const response = await fetch('{{ route('assessor.assessments.save-draft', $assessment) }}', {
                            method: 'POST',
                            body: formData,
                            headers: {
