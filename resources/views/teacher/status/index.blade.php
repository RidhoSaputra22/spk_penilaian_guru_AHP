<x-layouts.teacher>
    ...
    <x-ui.card class="mb-6" compact>
        <form action="{{ route('teacher.status.index') }}" method="GET"
            class="flex flex-col sm:flex-row gap-4 items-end">
            <div class="flex-1">
                <x-ui.select name="period_id" label="Filter Periode"
                    :options="$periods->mapWithKeys(fn($p) => [$p->id => $p->name . ' (' . $p->academic_year . ')'])->toArray()"
                    placeholder="Semua Periode" :selected="request('period_id')" />
            </div>
            <div class="flex-1">
                <x-ui.select name="status" label="Filter Status" :options="[
                        'pending' => 'Pending',
                        'draft' => 'Draft',
                        'in_progress' => 'Sedang Dinilai',
                        'submitted' => 'Submitted',
                        'finalized' => 'Final',
                    ]" placeholder="Semua Status" :selected="request('status')" />
            </div>
                            ...
                            <div class="flex items-center gap-2">
                                <div class="avatar placeholder">
                                    <div
                                        ...
                                        <span
                                            class="text-xs">{{ substr($assessment->assessor->user->name ?? '?', 0, 1) }}</span>
                                    </div>
                                </div>
