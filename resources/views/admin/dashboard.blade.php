<x-layouts.admin>
    ...
    <div class="stats stats-vertical lg:stats-horizontal shadow w-full mb-6 bg-base-100">
        ...
        <x-ui.stat title="Penilaian Selesai" :value="$completedAssessments ?? 0" :description="'dari ' . ($totalAssessments ?? 0) . ' total'" trend="up" trendValue="{{ $assessmentProgress ?? 0 }}%">
            <x-slot:icon>
                <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <x-ui.stat title="Periode Aktif" :value="$activePeriod->name ?? 'Tidak Ada'" description="{{ $activePeriod?->status ?? '-' }}">
            <x-slot:icon>
                <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                </svg>
                ...
                @forelse($assessmentsByStatus ?? [] as $status => $count)
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <progress class="progress w-32" value="{{ $count }}" max="{{ $totalAssessments ?? 1 }}"></progress>
                        </div>
                    </div>
                @empty
                    ...
                    <tbody>
                        @forelse($topTeachers ?? [] as $index => $teacher)
                            <tr>
                                    ...
                                    <div class="flex items-center gap-2">
                                        <div class="avatar placeholder">
                                            <div class="bg-neutral text-neutral-content rounded-full w-8">
                                                <span class="text-xs">{{ substr($teacher->teacher?->user?->name ?? 'G', 0, 1) }}</span>
                                            </div>
                                        </div>
                                ...
                                <td>
                                    <span class="font-mono font-bold text-primary">{{ number_format($teacher->final_score ?? 0, 2) }}</span>
                                </td>
                            </tr>
                        @empty
                            <tr>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="flex items-center gap-4 p-4 bg-base-200 rounded-lg">
                    ...
                    <div>
                        ...
                        <p class="text-2xl font-bold {{ ($ahpModel->consistency_ratio ?? 1) <= 0.1 ? 'text-success' : 'text-error' }}">
                ...
                <div class="flex items-center gap-4 p-4 bg-base-200 rounded-lg">
                    <div class="w-12 h-12 rounded-full {{ ($ahpModel->status ?? 'draft') === 'finalized' ? 'bg-success/20' : 'bg-warning/20' }} flex items-center justify-center">
