...
<div class="space-y-6">
        ...
        <div class="card-body">
            <form action="{{ route('admin.activity-logs.index') }}" method="GET" class="flex flex-wrap gap-4">
                <div class="form-control flex-1 min-w-[200px]">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari aktivitas atau nama..." class="input input-bordered" />
                </div>
                ...
                <div class="form-control w-40">
                    <input type="date" name="date_from" value="{{ request('date_from') }}" class="input input-bordered" placeholder="Dari" />
                </div>
                <div class="form-control w-40">
                    <input type="date" name="date_to" value="{{ request('date_to') }}" class="input input-bordered" placeholder="Sampai" />
                </div>
    ...
    <div class="card bg-base-100 shadow-sm">
        <div class="card-body">
            @forelse($logs as $log)
            <div class="flex gap-4 pb-6 {{ !$loop->last ? 'border-b border-base-200' : '' }} {{ !$loop->first ? 'pt-6' : '' }}">
                <!-- Avatar -->
                        ...
                        <span class="badge badge-ghost badge-sm">
                            @switch($log->action)
                                ...
                                @case('create_kpi_version')
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 mr-1 text-success" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                @case('delete_kpi_template')
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 mr-1 text-error" fill="none" viewBox="0 0 24 24" stroke="currentColor">
