...
<div class="space-y-6">
    ...
    <x-ui.card>
        <form method="GET" class="flex flex-col sm:flex-row gap-4">
            <div class="flex-1">
                <x-ui.input
                    type="search"
                    placeholder="Cari guru atau formulir..."
                    name="search"
            ...
            <div class="sm:w-48">
                <x-ui.select name="period">
                    ...
                    <option value="period1" {{ request('period') == 'period1' ? 'selected' : '' }}>Periode 1 - 2024</option>
                    <option value="period2" {{ request('period') == 'period2' ? 'selected' : '' }}>Periode 2 - 2024</option>
                </x-ui.select>
            </div>
            <div class="sm:w-48">
            ...
            <div class="flex gap-2">
                ...
                <x-ui.button variant="ghost" size="sm" onclick="window.location.href = '{{ route('admin.kpi-assignments.index') }}'">
                    Reset
                </x-ui.button>
            </div>
        </form>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <x-ui.stat
            ...
            value="24"
            description="Formulir yang ditugaskan"
        ...
        <x-ui.stat
            title="Dikerjakan"
            value="12"
            description="Sedang dalam progress"
                        ...
                        <tr>
                                ...
                                <div class="flex items-center gap-3">
                                    <div class="avatar placeholder">
                                        <div class="bg-primary text-primary-content w-10 rounded-full">
                                            <span class="text-sm">{{ substr($assignment->teacher_name ?? 'JS', 0, 2) }}</span>
                                        </div>
                                    </div>
                                    <div>
                                        <div class="font-semibold">{{ $assignment->teacher_name ?? 'John Smith' }}</div>
                                        <div class="text-sm text-base-content/70">{{ $assignment->employee_no ?? 'EMP001' }}</div>
                                    </div>
                                </div>
                            </td>
                            <td>
                                <div>
                                    <div class="font-semibold">{{ $assignment->form_name ?? 'Formulir Penilaian Kinerja' }}</div>
                                    <div class="text-sm text-base-content/70">v{{ $assignment->form_version ?? '1.0' }}</div>
                                </div>
                            </td>
                            <td>
                                <div class="text-sm">
                                    <div class="font-semibold">{{ $assignment->period_name ?? 'Periode 1 - 2024' }}</div>
                                    <div class="text-base-content/70">{{ $assignment->period_range ?? 'Jan - Jun 2024' }}</div>
                                </div>
                            </td>
                            <td>
                                @php
                            ...
                            <td>
                                ...
                                <div class="flex items-center gap-2">
                                    <progress class="progress progress-primary w-16" value="{{ $progress }}" max="100"></progress>
                            ...
                            <td>
                                <div class="dropdown dropdown-end">
                                    ...
                                    <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow-lg bg-base-100 rounded-box w-48">
                                        ...
                                        <li><a class="text-error" onclick="removeAssignmentModal.showModal()">Batalkan</a></li>
                                    </ul>
                                </div>
                            </td>
                        </tr>
                            ...
                            <td colspan="8" class="text-center py-8">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <div class="text-center">
                                        ...
                                        <p class="text-sm text-base-content/70">Tugaskan formulir KPI pertama kepada guru</p>
                                    </div>
                                    <x-ui.button variant="primary" size="sm" onclick="assignKpiModal.showModal()">
                                        Tugaskan KPI
                                    </x-ui.button>
...
<x-ui.modal id="removeAssignmentModal" title="Batalkan Penugasan">
    <form method="POST" action="#" class="space-y-4">
        ...
        <x-ui.alert type="warning">
            <strong>Peringatan!</strong> Tindakan ini akan membatalkan penugasan KPI dan menghapus semua progress yang sudah dibuat.
        </x-ui.alert>
