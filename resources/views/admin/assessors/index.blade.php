                ...
                <form action="{{ route('admin.assessors.index') }}" method="GET" class="flex flex-wrap gap-4">
                    <div class="form-control flex-1 min-w-[200px]">
                        <input type="text" name="search" value="{{ request('search') }}"
                            placeholder="Cari nama atau email..." class="input input-bordered" />
                    </div>
                    <div class="form-control w-48">
                        <select name="type" class="select select-bordered">
                            <option value="">Semua Tipe</option>
                            <option value="principal" {{ request('type') == 'principal' ? 'selected' : '' }}>Kepala
                                Sekolah</option>
                            <option value="supervisor" {{ request('type') == 'supervisor' ? 'selected' : '' }}>Pengawas
                            </option>
                            <option value="peer" {{ request('type') == 'peer' ? 'selected' : '' }}>Teman Sejawat
                            </option>
                        </select>
                    </div>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @php
                                    $progress = round(($assessor->completed_assignments / $assessor->total_assignments)
                                    * 100);
                                    @endphp
                                    <div class="flex items-center gap-2">
                                        <progress class="progress progress-success w-20" value="{{ $progress }}"
                                            max="100"></progress>
                                ...
                                <td class="text-right">
                                    <div class="dropdown dropdown-end">
                                        ...
                                        <ul tabindex="0"
                                            ...
                                            <li>
                                                <a
                                                    href="{{ route('admin.assessments.index') }}?assessor={{ $assessor->id }}">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none"
                                                        viewBox="0 0 24 24" stroke="currentColor">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                                            stroke-width="2"
