<x-layouts.admin>
    ...
    <x-slot:header>
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div class="flex gap-2">
                <x-ui.button type="ghost" href="{{ route('admin.criteria.sets.create') }}">
            ...
            <x-ui.card title="Set Kriteria">
                <div class="space-y-2">
                    @forelse($criteriaSets ?? [] as $set)
                        <a
                            href="{{ route('admin.criteria.index', ['set' => $set->id]) }}"
                            class="flex items-center justify-between p-3 rounded-lg hover:bg-base-200 transition-colors {{ ($currentSet->id ?? null) === $set->id ? 'bg-primary/10 border border-primary' : '' }}"
                        >
                            <div>
                                <div class="font-medium">{{ $set->name }}</div>
                                <div class="text-sm text-base-content/60">v{{ $set->version }}</div>
                            </div>
                    ...
                    @empty
                        <div class="text-center py-8 text-base-content/60">
                            ...
                            <x-ui.button type="primary" size="sm" href="{{ route('admin.criteria.sets.create') }}" class="mt-2">
                                Buat Set Pertama
                            </x-ui.button>
                        </div>
                    @endforelse
                ...
                <x-ui.card>
                    <div class="flex justify-between items-center mb-4">
                        <div>
                            ...
                            <p class="text-sm text-base-content/60">Version {{ $currentSet->version }} • {{ $criteriaNodes->count() }} kriteria</p>
                        </div>
                        <div class="flex gap-2">
                            @if(!$currentSet->locked_at)
                                <x-ui.button type="warning" size="sm" onclick="document.getElementById('lock-set-modal').showModal()">
                            ...
                            @endif
                            <x-ui.button type="ghost" size="sm" href="{{ route('admin.criteria.sets.edit', $currentSet) }}">
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            @if($criteria->description)
                                                <p class="text-sm text-base-content/60 mt-1">{{ Str::limit($criteria->description, 100) }}</p>
                                            @endif
                                        </div>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        ...
                                        @if(!$currentSet->locked_at)
                                            <div class="dropdown dropdown-end" @click.stop>
                                                ...
                                                <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow-lg bg-base-100 rounded-box w-48">
                                                    ...
                                                    <li><a onclick="addSubCriteria('{{ $criteria->id }}', '{{ $criteria->name }}')">Tambah Sub-kriteria</a></li>
                                                    <li><a class="text-error" onclick="deleteCriteria('{{ $criteria->id }}')">Hapus</a></li>
                                                </ul>
                                            </div>
                                        @endif
                                    </div>
                                ...
                                <div x-show="expandedNodes.includes('{{ $criteria->id }}')" x-collapse>
                                    <div class="border-t border-base-200 bg-base-200/30">
                                        @forelse($criteria->children as $subCriteria)
                                            <div class="flex items-center justify-between p-4 pl-12 border-b border-base-200 last:border-b-0 hover:bg-base-200/50">
                                                <div class="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <div class="flex items-center gap-2">
                                                            @if($subCriteria->code)
                                                                <x-ui.badge type="secondary" size="xs">{{ $subCriteria->code }}</x-ui.badge>
                                                            @endif
                                                            <span>{{ $subCriteria->name }}</span>
                                                        </div>
                                                        @if($subCriteria->description)
                                                            <p class="text-sm text-base-content/60">{{ Str::limit($subCriteria->description, 80) }}</p>
                                                        @endif
                                                    </div>
                                                </div>
                                                @if(!$currentSet->locked_at)
                                                    <div class="flex gap-1">
                                                        <a href="{{ route('admin.criteria.edit', $subCriteria) }}" class="btn btn-ghost btn-xs">
                                                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                                            </svg>
                                                        </a>
                                                        <button onclick="deleteCriteria('{{ $subCriteria->id }}')" class="btn btn-ghost btn-xs text-error">
                                                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        @if(!$currentSet->locked_at)
                                            <div class="p-2 pl-12">
                                                <button onclick="addSubCriteria('{{ $criteria->id }}', '{{ $criteria->name }}')" class="btn btn-ghost btn-sm btn-block justify-start">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
                                                    </svg>
                                                    Tambah Sub-kriteria
    ...
    <!-- Add Criteria Modal -->
    <x-ui.modal id="add-criteria-modal" title="Tambah Kriteria" size="md">
        <form method="POST" action="{{ route('admin.criteria.store') }}" class="space-y-4">
            @csrf
            ...
            <div id="parent-info" class="hidden">
                <x-ui.alert type="info">
                    Sub-kriteria dari: <strong id="parent-name-display"></strong>
                </x-ui.alert>
            </div>
    ...
    @if(isset($currentSet))
        <x-ui.modal id="lock-set-modal" title="Kunci Set Kriteria">
            ...
            <p class="text-sm text-warning mt-2">Setelah dikunci, kriteria tidak dapat diubah lagi.</p>
            <x-slot:actions>
                <form method="dialog">
                    <button class="btn btn-ghost">Batal</button>
                </form>
