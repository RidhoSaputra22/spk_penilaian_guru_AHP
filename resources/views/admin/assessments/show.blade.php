                                ...
                                <div class="flex items-center gap-3">
                                    <div class="avatar placeholder">
                                        <div class="bg-primary/10 text-primary rounded-full w-12">
                                            <span class="text-sm">{{ substr($assessment->teacher->user->name ?? '', 0, 2) }}</span>
                                        </div>
                                    </div>
                                    <div>
                                        <div class="font-medium text-base">{{ $assessment->teacher->user->name ?? '-' }}</div>
                                        <div class="text-sm text-base-content/60">NIP: {{ $assessment->teacher->nip ?? '-' }}</div>
                                        <div class="text-sm text-base-content/60">{{ $assessment->teacher->user->email ?? '-' }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="badge badge-{{ $assessment->status === 'submitted' ? 'success' : ($assessment->status === 'in_progress' ? 'warning' : 'ghost') }}">
                        ...
                        <div class="flex items-center gap-3">
                            <div class="avatar placeholder">
                                <div class="bg-secondary/10 text-secondary rounded-full w-12">
                                    <span class="text-sm">{{ substr($assessment->assessor->user->name ?? '', 0, 2) }}</span>
                                </div>
                            </div>
                            <div>
                                <div class="font-medium text-base">{{ $assessment->assessor->user->name ?? '-' }}</div>
                                <div class="text-sm text-base-content/60">{{ $assessment->assessor->user->email ?? '-' }}</div>
            ...
            <div class="card bg-base-100 shadow-sm">
                <div class="card-body">
                    ...
                    <div class="space-y-4 mt-4">
                        @php
                            $totalItems = $assessment->assignment->formVersion->template->sections->sum('items_count') ?? 0;
                        ...
                        <div class="alert alert-success">
                            ...
                            <span>Penilaian telah diselesaikan pada {{ $assessment->submitted_at->format('d M Y H:i') }}</span>
                        </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="card bg-base-100 shadow-sm">
                <div class="card-body">
                    ...
                    <div class="space-y-2 mt-4">
                        ...
                        <div class="badge badge-{{ $assessment->period->status === 'open' ? 'success' : 'ghost' }} badge-sm">
                ...
                <div class="card-body">
                    ...
                    <div class="space-y-2 mt-4">
                        <div class="text-sm">
                            ...
                            <div class="font-medium">{{ $assessment->assignment->formVersion->template->name ?? '-' }}</div>
                        </div>
                        <div class="text-sm">
                            <span class="text-base-content/60">Versi:</span>
                            <div class="font-medium">v{{ $assessment->assignment->formVersion->version_number ?? '1' }}</div>
                        </div>
                        <div class="text-sm">
