                ...
                <form action="{{ route('admin.teachers.index') }}" method="GET" class="flex flex-wrap gap-4">
                    <div class="form-control flex-1 min-w-[200px]">
                        <input type="text" name="search" value="{{ request('search') }}"
                            placeholder="Cari nama, NIP, NUPTK..." class="input input-bordered" />
                    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div class="stat bg-base-100 rounded-box shadow-sm">
                ...
                <div class="stat-value text-success">
                    {{ $teachers->where('current_assessment_status', 'completed')->count() }}</div>
            </div>
            <div class="stat bg-base-100 rounded-box shadow-sm">
                ...
                <div class="stat-value text-warning">
                    {{ $teachers->where('current_assessment_status', 'in_progress')->count() }}</div>
            </div>
            <div class="stat bg-base-100 rounded-box shadow-sm">
                ...
                <div class="stat-value">{{ $teachers->where('current_assessment_status', 'not_assigned')->count() }}
                </div>
            </div>
            @endif
        </div>
                                    ...
                                    <div class="flex items-center gap-3">
                                        <div class="avatar placeholder">
                                            <div
                                                ...
                                                <span
                                                    class="text-sm">{{ substr($teacher->user->name ?? '', 0, 2) }}</span>
                                            </div>
                                        </div>
                                        <div>
                                            <div class="font-medium">{{ $teacher->user->name ?? '-' }}</div>
                                            <div class="text-sm text-base-content/70">{{ $teacher->user->email ?? '-' }}
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="text-right">
                                    <div class="dropdown dropdown-end">
                                        ...
                                        <ul tabindex="0"
                                            ...
                                            <li>
                                                <a
                                                    href="{{ route('admin.assessments.index') }}?assign={{ $teacher->id }}">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none"
                                                        viewBox="0 0 24 24" stroke="currentColor">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                                            stroke-width="2"
